...
public class PresentsGenerator : MonoBehaviour
{
    ...
    // tree noise settings
    public NoiseSettings presentsNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public PresentsData GeneratePresentsData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        presentsNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        PresentsData presentsData = new PresentsData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GeneratePresentsNoise(chunkData, presentsNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        presentsData.presentsPositions = PresentsDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GeneratePresentsNoise(ChunkData chunkData, NoiseSettings presentsNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, presentsNoiseSettings);
