...
public class SugarCaneGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings sugarCaneNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public SugarCaneData GenerateSugarCaneData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        sugarCaneNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        SugarCaneData sugarCaneData = new SugarCaneData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateSugarCaneNoise(chunkData, sugarCaneNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        sugarCaneData.sugarCanePositions = SugarCaneDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GenerateSugarCaneNoise(ChunkData chunkData, NoiseSettings sugarCaneNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, sugarCaneNoiseSettings);
