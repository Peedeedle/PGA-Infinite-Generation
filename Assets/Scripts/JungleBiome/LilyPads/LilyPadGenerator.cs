...
public class LilyPadGenerator : MonoBehaviour
{
    ...
    // tree noise settings
    public NoiseSettings lilyPadNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public LilyPadData GenerateLilyPadData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        lilyPadNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        LilyPadData lilyPadData = new LilyPadData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateLilyPadNoise(chunkData, lilyPadNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        lilyPadData.lilyPadPositions = LilyPadDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GenerateLilyPadNoise(ChunkData chunkData, NoiseSettings lilyPadNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, lilyPadNoiseSettings);
