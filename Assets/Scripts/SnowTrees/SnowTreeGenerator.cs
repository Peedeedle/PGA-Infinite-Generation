...
public class SnowTreeGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings snowTreeNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public SnowTreeData GenerateSnowTreeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        snowTreeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        SnowTreeData snowTreeData = new SnowTreeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateSnowTreeNoise(chunkData, snowTreeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        snowTreeData.snowTreePositions = SnowTreeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GenerateSnowTreeNoise(ChunkData chunkData, NoiseSettings snowTreeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, snowTreeNoiseSettings);
