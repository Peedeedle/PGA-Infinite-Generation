////////////////////////////////////////////////////////////
// File: WMushroomTreeGenerator.cs
// Author: Jack Peedle
...
public class WMushroomTreeGenerator : MonoBehaviour
{
    ...
    public WMushroomTreeData GenerateWMushroomTreeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        wMushroomTreeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        WMushroomTreeData wMushroomTreeData = new WMushroomTreeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateWMushroomTreeNoise(chunkData, wMushroomTreeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        wMushroomTreeData.wMushroomTreePositions = WMushroomTreeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return wMushroomTreeData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateWMushroomTreeNoise(ChunkData chunkData, NoiseSettings wMushroomTreeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, wMushroomTreeNoiseSettings);
