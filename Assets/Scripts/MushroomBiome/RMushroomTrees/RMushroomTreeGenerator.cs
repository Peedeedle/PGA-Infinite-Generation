////////////////////////////////////////////////////////////
// File: RMushroomTreeGenerator.cs
// Author: Jack Peedle
...
public class RMushroomTreeGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings rMushroomTreeNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public RMushroomTreeData GenerateRMushroomTreeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        rMushroomTreeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        RMushroomTreeData rMushroomTreeData = new RMushroomTreeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateRMushroomTreeNoise(chunkData, rMushroomTreeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        rMushroomTreeData.rMushroomTreePositions = RMushroomTreeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return rMushroomTreeData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateRMushroomTreeNoise(ChunkData chunkData, NoiseSettings rMushroomTreeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, rMushroomTreeNoiseSettings);
