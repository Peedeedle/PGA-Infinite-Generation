...
public class RedBushGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings redBushNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public RedBushData GenerateRedBushData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        redBushNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        RedBushData redBushData = new RedBushData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateRedBushNoise(chunkData, redBushNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        redBushData.redBushPositions = RedBushDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GenerateRedBushNoise(ChunkData chunkData, NoiseSettings redBushNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, redBushNoiseSettings);
