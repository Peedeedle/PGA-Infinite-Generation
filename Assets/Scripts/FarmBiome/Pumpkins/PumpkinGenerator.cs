...
public class PumpkinGenerator : MonoBehaviour
{
    ...
    // tree noise settings
    public NoiseSettings pumpkinNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public PumpkinData GeneratePumpkinData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        pumpkinNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        PumpkinData pumpkinData = new PumpkinData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GeneratePumpkinNoise(chunkData, pumpkinNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        pumpkinData.pumpkinPositions = PumpkinDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GeneratePumpkinNoise(ChunkData chunkData, NoiseSettings pumpkinNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, pumpkinNoiseSettings);
