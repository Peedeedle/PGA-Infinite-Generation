...
public class BlueBushGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings blueBushNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public BlueBushData GenerateBlueBushData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        blueBushNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        BlueBushData blueBushData = new BlueBushData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateBlueBushNoise(chunkData, blueBushNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        blueBushData.blueBushPositions = BlueBushDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GenerateBlueBushNoise(ChunkData chunkData, NoiseSettings blueBushNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, blueBushNoiseSettings);
