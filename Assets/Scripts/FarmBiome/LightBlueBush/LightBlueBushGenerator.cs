////////////////////////////////////////////////////////////
// File: LightBlueBushGenerator.cs
// Author: Jack Peedle
...
public class LightBlueBushGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings lightBlueBushNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public LightBlueBushData GenerateLightBlueBushData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        lightBlueBushNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        LightBlueBushData lightBlueBushData = new LightBlueBushData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateLightBlueBushNoise(chunkData, lightBlueBushNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        lightBlueBushData.lightBlueBushPositions = LightBlueBushDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return lightBlueBushData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateLightBlueBushNoise(ChunkData chunkData, NoiseSettings lightBlueBushNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, lightBlueBushNoiseSettings);
