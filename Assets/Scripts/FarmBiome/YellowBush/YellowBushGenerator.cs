...
public class YellowBushGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings yellowBushNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public YellowBushData GenerateYellowBushData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        yellowBushNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        YellowBushData yellowBushData = new YellowBushData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateYellowBushNoise(chunkData, yellowBushNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        yellowBushData.yellowBushPositions = YellowBushDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return yellowBushData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateYellowBushNoise(ChunkData chunkData, NoiseSettings yellowBushNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, yellowBushNoiseSettings);
