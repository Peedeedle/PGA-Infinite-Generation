...
public class PurpleBushGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings purpleBushNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public PurpleBushData GeneratePurpleBushData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        purpleBushNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        PurpleBushData purpleBushData = new PurpleBushData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GeneratePurpleBushNoise(chunkData, purpleBushNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        purpleBushData.purpleBushPositions = PurpleBushDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return purpleBushData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GeneratePurpleBushNoise(ChunkData chunkData, NoiseSettings purpleBushNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, purpleBushNoiseSettings);
