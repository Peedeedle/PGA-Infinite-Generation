////////////////////////////////////////////////////////////
// File: PineappleCubeGenerator.cs
// Author: Jack Peedle
...
public class PineappleCubeGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings pineappleCubeNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public PineappleCubeData GeneratePineappleCubeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        pineappleCubeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        PineappleCubeData pineappleCubeData = new PineappleCubeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GeneratePineappleCubeNoise(chunkData, pineappleCubeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        pineappleCubeData.pineappleCubePositions = PineappleCubeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return pineappleCubeData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GeneratePineappleCubeNoise(ChunkData chunkData, NoiseSettings pineappleCubeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, pineappleCubeNoiseSettings);
