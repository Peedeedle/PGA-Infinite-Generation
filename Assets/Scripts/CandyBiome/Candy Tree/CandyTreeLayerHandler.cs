...
public class CandyTreeLayerHandler : BlockLayerHandler
{
    ...
    public static List<Vector3Int> candyTreeLeavesStaticLayout = new List<Vector3Int>() {
    ...
    protected override bool TryHandling(ChunkData chunkData, int x, int y, int z, int surfaceHeightNoise, Vector2Int mapSeedOffset) {
        ...
        if (surfaceHeightNoise < terrainHeightLimit && chunkData.candyTreeData.candyTreePositions.Contains(new Vector2Int(chunkData.worldPosition.x + x, chunkData.worldPosition.z + z))) {
            ...
            if (type == BlockType.CandyGrass) {
                ...
                foreach (Vector3Int leafPosition in candyTreeLeavesStaticLayout) {

                    // add the tree leaves solid to the vector 3 ints (X, Y and Z), surface height noise + 5 = i loop on line 106
                    chunkData.candyTreeData.candyTreeLeavesSolid.Add(new Vector3Int(x + leafPosition.x, surfaceHeightNoise + 5 
