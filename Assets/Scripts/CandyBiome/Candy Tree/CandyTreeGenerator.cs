...
public class CandyTreeGenerator : MonoBehaviour
{
    ...
    // tree noise settings
    public NoiseSettings candyTreeNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public CandyTreeData GenerateCandyTreeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        candyTreeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        CandyTreeData candyTreeData = new CandyTreeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateCandyTreeNoise(chunkData, candyTreeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        candyTreeData.candyTreePositions = CandyTreeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return candyTreeData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateCandyTreeNoise(ChunkData chunkData, NoiseSettings candyTreeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, candyTreeNoiseSettings);
