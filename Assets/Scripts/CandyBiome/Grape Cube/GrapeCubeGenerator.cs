...
public class GrapeCubeGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings grapeCubeNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public GrapeCubeData GenerateGrapeCubeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        grapeCubeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        GrapeCubeData grapeCubeData = new GrapeCubeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateGrapeCubeNoise(chunkData, grapeCubeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        grapeCubeData.grapeCubePositions = GrapeCubeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return grapeCubeData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateGrapeCubeNoise(ChunkData chunkData, NoiseSettings grapeCubeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, grapeCubeNoiseSettings);
