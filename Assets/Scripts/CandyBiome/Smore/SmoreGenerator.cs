...
public class SmoreGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings smoreNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public SmoreData GenerateSmoreData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        smoreNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        SmoreData smoreData = new SmoreData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateSmoreNoise(chunkData, smoreNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        smoreData.smorePositions = SmoreDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);
    ...
    private float[,] GenerateSmoreNoise(ChunkData chunkData, NoiseSettings smoreNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, smoreNoiseSettings);
