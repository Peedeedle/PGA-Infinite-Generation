...
public class OrangeCubeGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings orangeCubeNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public OrangeCubeData GenerateOrangeCubeData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        orangeCubeNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        OrangeCubeData orangeCubeData = new OrangeCubeData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateOrangeCubeNoise(chunkData, orangeCubeNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        orangeCubeData.orangeCubePositions = OrangeCubeDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return orangeCubeData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateOrangeCubeNoise(ChunkData chunkData, NoiseSettings orangeCubeNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, orangeCubeNoiseSettings);
