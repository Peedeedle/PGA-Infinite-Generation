...
public class GreenCaneGenerator : MonoBehaviour
{

    // tree noise settings
    public NoiseSettings greenCaneNoiseSettings;

    // domain warping
    public DomainWarping domainWarping;

    // Generate the tree data passing through the chunk data and a vector2int for the map offset
    public GreenCaneData GenerateGreenCaneData(ChunkData chunkData, Vector2Int mapSeedOffset) {

        // tree noise settings world offset = mapSeedOffset
        greenCaneNoiseSettings.worldOffset = mapSeedOffset;

        // treeData = new TreeData
        GreenCaneData greenCaneData = new GreenCaneData();

        // float array noiseData = generateTreeNoise method passing in (chunkData, treeNoiseSettings)
        float[,] noiseData = GenerateGreenCaneNoise(chunkData, greenCaneNoiseSettings);

        // tree positions = Data Proccessing method.findlocalMaxima passing in noise data and the chunks x and z world positions
        greenCaneData.greenCanePositions = GreenCaneDataProccessing.FindLocalMaxima(noiseData, chunkData.worldPosition.x, chunkData.worldPosition.z);

        // return the treeData
        return greenCaneData;

    }

    // private float array for GenerateTreeNoise (for each position in chunk) passing in (chunkData, treeNoiseSettings)
    private float[,] GenerateGreenCaneNoise(ChunkData chunkData, NoiseSettings greenCaneNoiseSettings) {
        ...
        for (int x = xMin; x < xMax; x++) {
            ...
            for (int z = zMin; z < zMax; z++) {
                ...
                noiseMax[xIndex, zIndex] = domainWarping.GenerateDomainNoise(x, z, greenCaneNoiseSettings);
